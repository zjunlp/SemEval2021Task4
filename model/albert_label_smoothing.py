...
class AlbertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 由于nn.Module 在 optimizer 包括的情况下回更新所有参数，所有我们使用register_buffer 来保证这个参数不会被optimizer更新。
...
class AlbertForMultipleChoiceWithLabelSmooth(AlbertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        这个方法对显存要求太高了，准备改写成为每次forward 只有一个choice
        input_ids [batch_size, num_choices, hidden_size]
        现在输入batch_size个句子，
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        num_choices = input_ids.shape[1] if input_ids is not None else inputs_embeds.shape[1]

        # 展开成[batch_size * choice_size,]
        # batch_size = input_ids.shape[0] 
        # outputs = torch.tensor([[]*batch_size], device = self.device)
        # for i in range(input_ids.shape[1]):
        #     batch_input_ids = input_ids[:,i,:] if input_ids is not None else None
        #     batch_attention_mask = attention_mask[:,i,:] if attention_mask is not None else None
        #     batch_token_type_ids = token_type_ids[:,i,:] if token_type_ids is not None else None
        #     batch_position_ids = position_ids[:,i,:] if position_ids is not None else None
        #     # t_outputs [batch_size, hidden_size]
        #     t_outputs = self.albert(
        #         batch_input_ids,
        ...
        #         torch.cat((outputs[j], t_outputs[1][j]), dim = 0, out=outputs[j])
        ...
        # t_input_ids = torch.split(input_ids,batch_size) if input_ids is not None else None
        # t_attention_mask = torch.split(attention_mask, batch_size) if attention_mask is not None else None
        # t_token_type_ids = torch.split(token_type_ids, batch_size) if token_type_ids is not None else None
        # t_position_ids = torch.split(position_ids, batch_size) if position_ids is not None else None
        # for i in range(input_ids.shape[0]):
        #     # batch_input_ids = input_ids[i:i+batch_size] if input_ids is not None else None
        #     # batch_attention_mask = attention_mask[i:i+batch_size,:] if attention_mask is not None else None
        #     # batch_token_type_ids = token_type_ids[i:i+batch_size,:] if token_type_ids is not None else None
        #     # batch_position_ids = position_ids[i:i+batch_size,:] if position_ids is not None else None

        #     a = t_input_ids[i]
        #     b = t_attention_mask[i]
        #     c = t_token_type_ids[i]
        #     d = t_position_ids[i]
        #     import IPython; IPython.embed(); exit(1)
        #     t_outputs = self.albert(
        #         a,
        ...
        #         return_dict=return_dict,
        #     )
        #     outputs = torch.cat((outputs, t_outputs[1]), dim=0)
