...
class RobertaForMultipleChoiceWithLabelSmooth(RobertaPreTrainedModel):
    authorized_missing_keys = [r"position_ids"]
...
class RobertaForSequenceClassificationWithLabelSmooth(RobertaPreTrainedModel):
    authorized_missing_keys = [r"position_ids"]
    ...
    def forward(
        ...
        self.num_labels = input_ids.shape[1]
...
class RobertaForMultipleChoiceModifed(RobertaPreTrainedModel):
    authorized_missing_keys = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        # hope it will work
        self.origin_cls = nn.Linear(config.hidden_size * 5, 5)
    ...
    def forward(
        ...
        #         sequence_output * flat_attention_mask.unsqueeze(-1), dim=1
        #     ) / torch.clamp(torch.sum(flat_attention_mask, dim=1, keepdims=True), min=1e-9)
        pooled_output = outputs[1]

        # import IPython; IPython.embed(); exit(1)
        pooled_output = self.dropout(pooled_output)
        t = pooled_output.view(-1, num_choices, self.config.hidden_size)
        t = torch.cat((t[:,0,:],t[:,1,:],t[:,2,:],t[:,3,:],t[:,4,:]), dim = 1)

        reshaped_logits = self.origin_cls(t)
        # logits = self.classifier(pooled_output)
...
class RobertaForMultipleChoiceEnhanced(RobertaPreTrainedModel):
    authorized_missing_keys = [r"position_ids"]
